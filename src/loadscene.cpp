...
bool LoadScene(SceneT &scene, CameraT &cam, OptionsT &options,
  ...
  while (ist.good()) {
    ...
    if (EqualStr(line, "ball")) {
      ...
      CylinderT *cyl = new CylinderT(xforms.top());
      cyl->m_mat = states.top().m_mat;
      scene.AddGel(cyl);
      ...
      int nread = sscanf(line.c_str(), "%s %f %f %f %f %f %f %f %f %f", dummy,
                         &pts[0].x(), &pts[0].y(), &pts[0].z(), &pts[1].x(),
                         &pts[1].y(), &pts[1].z(), &pts[2].x(), &pts[2].y(),
                         &pts[2].z());
      if (nread != 10) {
        cerr << "ERROR: " << line << endl;
        continue;
      ...
      XVec3f color;
      int nread =
          sscanf(line.c_str(), "%s %f %f %f %f %f %f", dummy, &center.x(),
                 &center.y(), &center.z(), &color.x(), &color.y(), &color.z());
      if (nread != 7) {
        cerr << "ERROR: " << line << endl;
        continue;
      }
      ...
      float width;
      XVec3f color;
      int nread = sscanf(line.c_str(), "%s %f %f %f %f", dummy, &width,
      ...
      XVec3f c;
      int nread =
          sscanf(line.c_str(), "%s %f %f %f", dummy, &c.x(), &c.y(), &c.z());
      if (nread != 4) {
        cerr << "ERROR: " << line << endl;
      }
      ...
      int nread = sscanf(line.c_str(), "%s %f %f %f", dummy, &cam.m_eyepos.x(),
                         &cam.m_eyepos.y(), &cam.m_eyepos.z());
      if (nread != 4) {
        cerr << "ERROR: " << line << endl;
      }
    } else if (EqualStr(line, "eyedir")) {
      int nread = sscanf(line.c_str(), "%s %f %f %f", dummy, &cam.m_eyedir.x(),
                         &cam.m_eyedir.y(), &cam.m_eyedir.z());
      if (nread != 4) {
        cerr << "ERROR: " << line << endl;
      }
    } else if (EqualStr(line, "eyeup")) {
      int nread = sscanf(line.c_str(), "%s %f %f %f", dummy, &cam.m_eyeup.x(),
                         &cam.m_eyeup.y(), &cam.m_eyeup.z());
      if (nread != 4) {
        cerr << "ERROR: " << line << endl;
      }
    } else if (EqualStr(line, "wdist")) {
      int nread = sscanf(line.c_str(), "%s %f", dummy, &cam.m_wdist);
      if (nread != 2) {
        cerr << "ERROR: " << line << endl;
      }
    } else if (EqualStr(line, "fovy_deg")) {
      int nread = sscanf(line.c_str(), "%s %f", dummy, &cam.m_fovy);
      if (nread != 2) {
        cerr << "ERROR: " << line << endl;
      }
      cam.m_fovy *= float(M_PI) / 180;
    } else if (EqualStr(line, "nx")) {
      int nread = sscanf(line.c_str(), "%s %d", dummy, &cam.m_nx);
      if (nread != 2) {
        cerr << "ERROR: " << line << endl;
      }
    } else if (EqualStr(line, "ny")) {
      int nread = sscanf(line.c_str(), "%s %d", dummy, &cam.m_ny);
      if (nread != 2) {
        cerr << "ERROR: " << line << endl;
      }
    } else if (EqualStr(line, "max_recursion")) {
      int nread =
          sscanf(line.c_str(), "%s %d", dummy, &options.m_max_recursion);
      if (nread != 2) {
        cerr << "ERROR: " << line << endl;
      }
    } else if (EqualStr(line, "aasample")) {
      int nread = sscanf(line.c_str(), "%s %d", dummy, &options.m_aasample);
      if (nread != 2) {
        cerr << "ERROR: " << line << endl;
      }
      ...
      XVec3f axis;
      float angle;
      int nread = sscanf(line.c_str(), "%s %f %f %f %f", dummy, &angle,
      ...
      XVec3f delta;
      int nread = sscanf(line.c_str(), "%s %f %f %f", dummy, &delta.x(),
                         &delta.y(), &delta.z());
      if (nread != 4) {
        cerr << "ERROR: " << line << endl;
      }
