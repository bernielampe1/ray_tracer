...
// 3x4 matrix class
template <class real_t> class XFormT {
public:
  XFormT(void) {}
  ...
  const real_t &at(int i, int j) const { return m_a[(i << 2) + j]; }

  /// access to the (i,j) element
  real_t &at(int i, int j) { return m_a[(i << 2) + j]; }
  ...
  XFormT<real_t> Inverse() const {
    ...
    xf.m_a[11] =
        -xf.m_a[8] * m_a[3] - xf.m_a[9] * m_a[7] - xf.m_a[10] * m_a[11];
  ...
  XFormT<real_t> &operator*=(const XFormT<real_t> &xf) {
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      m_a[4 * k + 0] =
          row(0) * xf.m_a[0] + row(1) * xf.m_a[4] + row(2) * xf.m_a[8];
      m_a[4 * k + 1] =
          row(0) * xf.m_a[1] + row(1) * xf.m_a[5] + row(2) * xf.m_a[9];
      m_a[4 * k + 2] =
          row(0) * xf.m_a[2] + row(1) * xf.m_a[6] + row(2) * xf.m_a[10];
      m_a[4 * k + 3] = row(0) * xf.m_a[3] + row(1) * xf.m_a[7] +
                       row(2) * xf.m_a[11] + row(3);
    }
    return *this;
  }
  // XVecT<real_t> o
