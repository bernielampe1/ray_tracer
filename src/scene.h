...
public:
  SceneT() : m_bkcolor(0.0f, 0.0f, 0.30f) {}
  ...
  const XVec3f &BackgroundColor() const { return m_bkcolor; }
  /// sets the background color
  void BackgroundColor(const XVec3f &color) { m_bkcolor = color; }

  LightCt::const_iterator BeginLights() const { return m_lights.begin(); }
  LightCt::const_iterator EndLights() const { return m_lights.end(); }

  void AddLight(ILight *light) { m_lights.push_back(light); }
  ...
  void print_lights() const {
    ...
    for (LightCt::const_iterator i = BeginLights(); i != EndLights(); ++i) {
