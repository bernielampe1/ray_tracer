...
public:
  ...
  struct vertex_t {
    vertex_t(const XVec3f &_pos) : m_pos(_pos) {}
    XVec3f m_pos;
    XVec3f m_normal;
  };
  ...
  struct triangle_t {
    triangle_t(const XVec3i &_inds) : m_inds(_inds) {}
    XVec3i m_inds;
    XVec3f m_normal;
  };
...
public:
  ...
  virtual bool Intersect(const ray_t &wray, hitinfo_t &hit) const {
    ...
    // the bounding volume testing
    hitinfo_t thit;
    if (!boundingSphere.Intersect(wray, thit)) {
      return (false);
    }
    ...
    for (ti = m_fs.begin(); ti != m_fs.end(); ++ti) {
      ...
      if (TriangleT::_intersect(m_vs[(*ti).m_inds[0]].m_pos,
                                ...
                                m_vs[(*ti).m_inds[2]].m_pos, ray, t, u, v)) {
    ...
    hit.m_t = tmin;
    hit.m_pos = (1.0f - umin - vmin) * m_vs[(*timin).m_inds[0]].m_pos +
    ...
    case PHONG_SHADE:
      ...
      XVec3f n0 = m_vs[(*timin).m_inds(0)].m_normal;
      XVec3f n1 = m_vs[(*timin).m_inds(1)].m_normal;
      XVec3f n2 = m_vs[(*timin).m_inds(2)].m_normal;
      hit.m_normal = n0 + umin * (n1 - n0) + vmin * (n2 - n0);
      break;
    }
    ...
    while (ist.good()) {
      ...
      if (line[0] == 'v') {
        if (line[1] == 'n') {
          // read normal here
          XVec3f normal;
          int nr = sscanf(line.c_str(), "vn %f %f %f", &normal.x(), &normal.y(),
                          &normal.z());
          if (nr != 3) {
  ...
  void AddVertex(const XVec3f &pos) { m_vs.push_back(vertex_t(pos)); }
  /// Adds a triangle to the triangle set
  void AddTriangle(const XVec3i &inds) { m_fs.push_back(triangle_t(inds)); }

  /// Initializes bounding volume for the mesh
  void ComputeBV() {
    // initialize the bounding sphere
    XVec3f center;
    float radius, minX, maxX, minY, maxY, minZ, maxZ;

    // compute axially aligned bounding box
    minX = maxX = m_vs[0].m_pos(0);
    minY = maxY = m_vs[0].m_pos(1);
    minZ = maxZ = m_vs[0].m_pos(2);
    for (unsigned i = 1; i < m_vs.size(); i++) {
      if (minX > m_vs[i].m_pos(0))
        minX = m_vs[i].m_pos(0);
      else if (maxX < m_vs[i].m_pos(0))
        maxX = m_vs[i].m_pos(0);

      if (minY > m_vs[i].m_pos(1))
        minY = m_vs[i].m_pos(1);
      else if (maxY < m_vs[i].m_pos(1))
        maxY = m_vs[i].m_pos(1);

      if (minZ > m_vs[i].m_pos(2))
        minZ = m_vs[i].m_pos(2);
      else if (maxZ < m_vs[i].m_pos(2))
        maxZ = m_vs[i].m_pos(2);
    }

    // estimate sphere from aa bounding box
    center =
        XVec3f((maxX + minX) / 2.0, (maxY + minY) / 2.0, (maxZ + minZ) / 2.0);
    radius = center.dist(XVec3f(maxX, maxY, maxZ));
    boundingSphere = BallT(radius, center, m_xf);
  }

  /// Set a particular shading mode: flat or phong
  void SetShading(int _shade) { m_shade = _shade; }
