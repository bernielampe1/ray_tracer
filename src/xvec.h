...
template <int dim, class real_type> class XVec {
public:
  // Constructors.
  XVec() {}
  ...
  template <class oreal_type> explicit XVec(const XVec<dim, oreal_type> &c) {
    for (int i = 0; i < dim; ++i)
  ...
  real_type &x() { return operator()(0); }

  real_type x() const { return m_v[0]; }

  real_type &y() { return m_v[1]; }

  real_type y() const { return m_v[1]; }

  real_type &z() { return m_v[2]; }

  real_type z() const { return m_v[2]; }
  ...
  // L2-norm of a vector.
  real_type l2(void) const { return sqrtf(dot()); }

  // Euclidean distance between two points.
  real_type dist(const XVec &c) const { return (*this - c).l2(); }
...
template <int dim, class real_type>
std::istream &operator>>(std::istream &is, XVec<dim, real_type> &f) {
