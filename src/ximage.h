...
// Image class
template <class IPixelT> class XImageT {
public:
  ...
  const IPixelT &at(int i, int j) const { return m_p[i + j * m_nx]; }

  IPixelT &at(int i, int j) { return m_p[i + j * m_nx]; }

  size_t width() const { return m_nx; }

  size_t height() const { return m_ny; }

  IPixelT *row_pointer(int y) const { return m_p + (m_ny - y - 1) * m_nx; }
